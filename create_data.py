...
def action(board, player1, player2):
	row = col = None
	if player1.state:
		row, col, _ = set_move_bot(board, player1.chess, player2.chess)
		...
		row, col, _ = set_move_bot(board, player2.chess, player1.chess)
		board[row][col] = player2.chess
		with open('data/data_minimax_v1.txt', mode='a') as f:
			f.write(str(board) + ",\n")
	player1.set_state()
	player2.set_state()
